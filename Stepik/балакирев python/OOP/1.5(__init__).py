...
#     pass
#
# elements = [(Line, Rect, Ellipse)[randint(0, 2)](randint(-100, 100), randint(-100, 100), randint(-100, 100), randint(-100, 100)) for _
#     in range(217)]
#
# for obj in elements:
...
#         return print(' '.join(map(str, self.data)) if self.is_show else 'Отображение данных закрыто')
#
#     def show_graph(self):
...
#         return print(
#             f"Графическое отображение данных: {' '.join(map(str, self.data))}" if self.is_show is True else 'Отображение данных закрыто')
#
#     def show_bar(self):
#         """для отображения данных в виде столбчатой диаграммы (метод выводит в консоль сообщение:
#         "Столбчатая диаграмма: <строка из чисел следующих через пробел>")"""
#         return print(
#             f"Столбчатая диаграмма: {' '.join(map(str, self.data))}" if self.is_show is True else 'Отображение данных закрыто')
#
...
# для класса MotherBoard: name - наименование; cpu - ссылка на объект класса CPU; total_mem_slots = 4 - общее число слотов
# памяти (атрибут прописывается с этим значением и не меняется); mem_slots - список из объектов класса Memory
...
#         # self.mem_slots = list(Memory.__dict__.values())
#
#     def get_config(self):
...
# Table, TV, Notebook и Cup). Изначально этот список должен быть пустым.
...
#
# cart = Cart()
# cart.add(TV('LG', '25 000'))
# cart.add(TV('Samsung', '35 000'))
# cart.add(Table('Computer table', '15 000'))
# cart.add(Notebook('Lenovo', '60 000'))
# cart.add(Notebook('Apple', '210 000'))
# cart.add(Cup('Get OFF', '800'))
# print(cart.get_list())
...
# строка в первом объекте, вторая - во втором и  т.д.). На первый добавленный объект класса ListObject должна ссылаться переменная head_obj.
