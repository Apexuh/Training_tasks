# # пользовательский класс изначально наследуется от базового Object
#
# class Geom: # python 3.0 : Geom, < 3.0: Geom(object)
#     pass
#
# class Line(Geom):
#     pass
#
# g = Geom()
# print(g) #<__main__.Geom object at 0x108622740>
# l = Line()
# print(l.__class__) #<class '__main__.Line'>
# print(issubclass(Line, Geom)) #True. line дочерний класс класса geom, с экземплярами это не работает, только с классами
# print(isinstance(l, Geom)) #True
#
# #особенность языка: int, float, str ...   все являются классами и соответственно дочерними классами класса object
# print(issubclass(int, object)) #true
# print(issubclass(list, object)) #true
#
# # ---->
# # можем переопределить функции классов
# # --->
# class Vector(list):
#     pass                    #[1, 2, 3]
...
#         if len(other):
#                 return Tuple(tuple(self) + tuple(other)) if not isinstance(other, tuple) else Tuple(tuple(self) + other)
...
# Подвиг 7 (на повторение). Необходимо в программе объявить класс VideoItem для представления одного видео (например, в youtube).
...
# Затем, нужно создать класс для формирования оценки видео в баллах от 0 до 5. Для этого нужно объявить еще один класс с именем VideoRating,
# объекты которого создаются командой:
# rating = VideoRating()
# В каждом объекте класса VideoRating должен быть локальный приватный атрибут с именем __rating, содержащий целое число от 0 до 5 (по умолчанию 0).
# А для записи и считывания значения из этого приватного атрибута должно быть объект-свойство (property) с именем rating.
# Так как атрибут __rating - это целое число в диапазоне [0; 5], то в момент присвоения ему какого-либо значения необходимо проверять,
...
# Подвиг 9 (на повторение). Объявите в программе базовый класс с именем IteratorAttrs для перебора всех
# локальных атрибутов объектов класса. Напомню, что для этого используются два магических метода:
...
# размер ОЗУ (памяти), как целое число. В каждом объекте класса SmartPhone должны создаваться соответствующие локальные
