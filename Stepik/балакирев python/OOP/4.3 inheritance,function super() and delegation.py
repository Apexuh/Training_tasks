...
#         self.x2 = x2
#         self.y2 = y2
#         print('Initializator Line class')
#
#     def draw(self):
#         print('Painting line')
#
...
#         self.y2 = y2
#         self.fill = fill
#         print('Initializator Rect class')
#
#     def draw(self):
#         print('Painting Rect')

# l = Line() #Initializator Geom class if Line have not self init func
# если в geom есть метод, а в line он же, то это переопределение
# если в geom нет, а в line есть, то это расширение

# l = Line(0, 0, 10, 20)
# print(l.__dict__)   #{'x1': 0, 'y1': 0, 'x2': 10, 'y2': 20}


# уберем дублирование кода
# class Geom:
#     name = 'Geom'
#
#     def __init__(self, x1, y1, x2, y2):
#         print(f'Initializator Geom class for class {self.__class__}')
#         self.x1 = x1
#         self.y1 = y1
#         self.x2 = x2
#         self.y2 = y2
...
#         super().__init__(self, x1, y1, x2, y2) # ссылка на базовый класс(можно было Geom().__init....) Это назвается делегированием
#         print('Initializator Rect class')
#         self.fill = fill
#
#     def draw(self):
...
# Подвиг 3. Ранее вы уже использовали делегирование методов, когда вызывали инициализатор базового класса через функцию super().
# Чаще всего делегирование в Python связано с вызовом магических методов базовых классов (так как их имена нельзя менять). Выполним такой пример.
# Объявите в программе базовый класс с именем Book, объекты которого создаются командой:
# book = Book(title, author, pages, year)
# где title - заголовок книги (строка); author - автор книги (строка); pages - число страниц (целое число); year - год издания (целое число).
...
# где дополнительные параметры size - размер книги в байтах (целое число); frm - формат книги (строка: 'pdf', 'doc', 'fb2', 'txt').
# В каждом объекте класса DigitBook должны формироваться соответствующие локальные атрибуты: title, author, pages, year, size, frm.
...
# Небольшая справка.
# Для реализации декоратора с параметрами на уровне класса в инициализаторе __init__(self, methods) прописываем параметр для декоратора,
# а магический метод __call__() объявляем для декорирования функции:
# class Handler:
...
# В классе SoftList следует объявить необходимые магические методы так, чтобы при обращении к несуществующему элементу (по индексу)
...
# Подвиг 10 (на повторение). Объявите базовый класс с именем ItemAttrs, который бы позволял обращаться
# к локальным атрибутам объектов дочерних классов по индексу. Для этого в классе ItemAttrs нужно переопределить следующие методы:
...
class ItemAttrs:
    ...
    def __setitem__(self, key, value):
        d = list(self.__dict__.values())
        key_n = list(self.__dict__)[key]
        if not d[key]:
            raise KeyError
        self.__dict__[key_n] = value
